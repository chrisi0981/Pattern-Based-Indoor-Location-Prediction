...
def Predict_Occupancy(user,pattern_length,look_ahead,ensemble,processes):
	...
	result = dbHandler.select("SELECT pattern_members,probability,specificity,TRIM(TRAILING ',' FROM SUBSTRING_INDEX(pattern_members, ',',-2)),count FROM Pattern_Base.Pattern_Base_%i" % (user))
	...
	last_date = ""
	sd_comb_old = [[] for k in range(pattern_length-1)]
	pd_comb_old = [[] for k in range(pattern_length-1)]
	p2d_comb_old = [[] for k in range(pattern_length-1)]
	sw_comb_old = [[] for k in range(pattern_length-1)]
	pw_comb_old = [[] for k in range(pattern_length-1)]
	p2w_comb_old = [[] for k in range(pattern_length-1)]
	sm_comb_old = [[] for k in range(pattern_length-1)]
	...
	sd_old_patterns = [[] for k in range(pattern_length-1)]
	pd_old_patterns = [[] for k in range(pattern_length-1)]
	p2d_old_patterns = [[] for k in range(pattern_length-1)]
	sw_old_patterns = [[] for k in range(pattern_length-1)]
	pw_old_patterns = [[] for k in range(pattern_length-1)]
	p2w_old_patterns = [[] for k in range(pattern_length-1)]
	sm_old_patterns = [[] for k in range(pattern_length-1)]
				...
				if len(window) < look_ahead:
					...
					if pattern_length == 1:
						...
						if window[-1][11] != '':
							...
							if last_date != row[2]:
								"""
									Reinitialize lists at start of new day
								"""
								sd_comb_old = [[] for k in range(pattern_length-1)]
								pd_comb_old = [[] for k in range(pattern_length-1)]
								p2d_comb_old = [[] for k in range(pattern_length-1)]
							...
							#pool_result.append(pool.apply_async(Create_Pattern_Combinations, (patterns,same_day_indices,data_point_patterns,pattern_length,sd_comb_old,sd_unique_patterns,sd_old_patterns,valid_patterns,)))
							#1							
							pd_comb,pd_comb_old,pd_unique_patterns,pd_old_patterns = Create_Pattern_Combinations(patterns,previous_day_indicies,data_point_patterns,pattern_length,pd_comb_old,pd_unique_patterns,pd_old_patterns,valid_patterns)
							#pool_result.append(pool.apply_async(Create_Pattern_Combinations, (patterns,previous_day_indicies,data_point_patterns,pattern_length,pd_comb_old,pd_unique_patterns,pd_old_patterns,valid_patterns,)))
							#2							
							p2d_comb,p2d_comb_old,p2d_unique_patterns,p2d_old_patterns = Create_Pattern_Combinations(patterns,previous_2day_indicies,data_point_patterns,pattern_length,p2d_comb_old,p2d_unique_patterns,p2d_old_patterns,valid_patterns)
							#pool_result.append(pool.apply_async(Create_Pattern_Combinations, (patterns,previous_2day_indicies,data_point_patterns,pattern_length,p2d_comb_old,p2d_unique_patterns,p2d_old_patterns,valid_patterns,)))
							
							#sd_comb,sd_comb_old,sd_unique_patterns,sd_old_patterns = pool_result[0].get()
							#pd_comb,pd_comb_old,pd_unique_patterns,pd_old_patterns = pool_result[1].get()
							#p2d_comb,p2d_comb_old,p2d_unique_patterns,p2d_old_patterns = pool_result[2].get()
							
							pool_result = []
							
							#3							
							sw_comb,sw_comb_old,sw_unique_patterns,sw_old_patterns = Create_Pattern_Combinations(patterns,same_week_indices,data_point_patterns,pattern_length,sw_comb_old,sw_unique_patterns,sw_old_patterns,valid_patterns)	
							#pool_result.append(pool.apply_async(Create_Pattern_Combinations, (patterns,same_week_indices,data_point_patterns,pattern_length,sw_comb_old,sw_unique_patterns,sw_old_patterns,valid_patterns,)))
							#4							
							pw_comb,pw_comb_old,pw_unique_patterns,pw_old_patterns = Create_Pattern_Combinations(patterns,previous_week_indicies,data_point_patterns,pattern_length,pw_comb_old,pw_unique_patterns,pw_old_patterns,valid_patterns)
							#pool_result.append(pool.apply_async(Create_Pattern_Combinations, (patterns,previous_week_indicies,data_point_patterns,pattern_length,pw_comb_old,pw_unique_patterns,pw_old_patterns,valid_patterns,)))
							#5							
							p2w_comb,p2w_comb_old,p2w_unique_patterns,p2w_old_patterns = Create_Pattern_Combinations(patterns,previous_2week_indicies,data_point_patterns,pattern_length,p2w_comb_old,p2w_unique_patterns,p2w_old_patterns,valid_patterns)
							#pool_result.append(pool.apply_async(Create_Pattern_Combinations, (patterns,previous_2week_indicies,data_point_patterns,pattern_length,p2w_comb_old,p2w_unique_patterns,p2w_old_patterns,valid_patterns,)))
							
							#sw_comb,sw_comb_old,sw_unique_patterns,sw_old_patterns = pool_result[0].get()
							#pw_comb,pw_comb_old,pw_unique_patterns,pw_old_patterns = pool_result[1].get()
							#p2w_comb,p2w_comb_old,p2w_unique_patterns,p2w_old_patterns = pool_result[2].get()
							
							#6							
							sm_comb,sm_comb_old,sm_unique_patterns,sm_old_patterns = Create_Pattern_Combinations(patterns,same_month_indices,data_point_patterns,pattern_length,sm_comb_old,sm_unique_patterns,sm_old_patterns,valid_patterns)							
							...
							print ',93,929,' in pd_comb[0]
							print ',93,929,' in p2d_comb[0]
							print ',93,929,' in valid_patterns
							
							#print p2d_comb[0]
							
							for l in range(pattern_length-1):
								sd_prob.append([(k,valid_patterns[k][0]*weights[0],0,valid_patterns[k][7],singular_patterns_occ[valid_patterns[k][7]-1],valid_patterns[k][0]) for k in sd_comb[l] if valid_patterns[k][0] != 0])
								pd_prob.append([(k,valid_patterns[k][1]*weights[1],1,valid_patterns[k][7],singular_patterns_occ[valid_patterns[k][7]-1],valid_patterns[k][1]) for k in pd_comb[l] if valid_patterns[k][1] != 0])
								p2d_prob.append([(k,valid_patterns[k][2]*weights[2],2,valid_patterns[k][7],singular_patterns_occ[valid_patterns[k][7]-1],valid_patterns[k][2]) for k in p2d_comb[l] if valid_patterns[k][2] != 0])
								
								sw_prob.append([(k,valid_patterns[k][3]*weights[3],3,valid_patterns[k][7],singular_patterns_occ[valid_patterns[k][7]-1],valid_patterns[k][3]) for k in sw_comb[l] if valid_patterns[k][3] != 0])
								pw_prob.append([(k,valid_patterns[k][4]*weights[4],4,valid_patterns[k][7],singular_patterns_occ[valid_patterns[k][7]-1],valid_patterns[k][4]) for k in pw_comb[l] if valid_patterns[k][4] != 0])
								p2w_prob.append([(k,valid_patterns[k][5]*weights[5],5,valid_patterns[k][7],singular_patterns_occ[valid_patterns[k][7]-1],valid_patterns[k][5]) for k in p2w_comb[l] if valid_patterns[k][5] != 0])
								
								sm_prob.append([(k,valid_patterns[k][6]*weights[6],6,valid_patterns[k][7],singular_patterns_occ[valid_patterns[k][7]-1],valid_patterns[k][6]) for k in sm_comb[l] if valid_patterns[k][6] != 0])
							...
							if int(row[1]) < 1333321210:
								...
								for current_target in targets:
									...
									target_tmp.append([sd_prob[k][l][5] for k in range(len(sd_prob)) if len(sd_prob[k]) > 0 for l in range(len(sd_prob[k])) if sd_prob[k][l][4] == current_target and sd_prob[k][l][5] >= cut_off])
									
									# One Adjacent Day Probability
									target_tmp.append([pd_prob[k][l][5] for k in range(len(pd_prob)) if len(pd_prob[k]) > 0 for l in range(len(pd_prob[k])) if pd_prob[k][l][4] == current_target and pd_prob[k][l][5] >= cut_off])
									
									# Two Adjacent Day Probability
									target_tmp.append([p2d_prob[k][l][5] for k in range(len(p2d_prob)) if len(p2d_prob[k]) > 0 for l in range(len(p2d_prob[k])) if p2d_prob[k][l][4] == current_target and p2d_prob[k][l][5] >= cut_off])
									
									# Same Week Probability
									target_tmp.append([sw_prob[k][l][5] for k in range(len(sw_prob)) if len(sw_prob[k]) > 0 for l in range(len(sw_prob[k])) if sw_prob[k][l][4] == current_target and sw_prob[k][l][5] >= cut_off])	
									
									# One Adjacent Week Probability
									target_tmp.append([pw_prob[k][l][5] for k in range(len(pw_prob)) if len(pw_prob[k]) > 0 for l in range(len(pw_prob[k])) if pw_prob[k][l][4] == current_target and pw_prob[k][l][5] >= cut_off])
									
									# Two Adjacent Week Probability
									target_tmp.append([p2w_prob[k][l][5] for k in range(len(p2w_prob)) if len(p2w_prob[k]) > 0 for l in range(len(p2w_prob[k])) if p2w_prob[k][l][4] == current_target and p2w_prob[k][l][5] >= cut_off])
									
									# Same Month Probability
									target_tmp.append([sm_prob[k][l][5] for k in range(len(sm_prob)) if len(sm_prob[k]) > 0 for l in range(len(sm_prob[k])) if sm_prob[k][l][4] == current_target and sm_prob[k][l][5] >= cut_off])
